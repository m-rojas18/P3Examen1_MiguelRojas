...
void PC::ping(string ip_buscado, vector<PC*> lista_PCs){
    ...
    for (int i = 0; i < lista_PCs.size(); i++){
        PC* pc_temp = lista_PCs[i];
        string ip_prueba = pc_temp->getDireccion_IP();

        if(ip_buscado == ip_prueba){
            //La computadora existe
            verificar_pc = false;
        } 
    }
    ...
    if(verificar_pc == false){//Existe la pc buscada
        ...
        //No existe la PC, ping inexistente
         cout << "\nPinging to " << ip_buscado << " with 32 bytes of data:" <<endl
                ...
                << endl
                << "Ping statistics for " << ip_buscado << ": " << endl
                << "   Packets: Sent = 4, Recieved = 0, Lost = 4 (100% loss)" << endl << endl;
